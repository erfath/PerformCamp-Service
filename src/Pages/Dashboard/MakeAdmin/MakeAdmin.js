...
const MakeAdmin = () => {
  ...
  useEffect(() => {
    fetch('https://performcamp-server.onrender.com/user')
      .then(res => res.json())
  ...
  const handleEdit = (email) => {
    ...
    fetch(`https://performcamp-server.onrender.com/user_admin/${email}`, {
      method: "PUT",
      headers: {
  ...
  const handleDelete = (email) =>{
    ...
    fetch(`https://performcamp-server.onrender.com/user/${email}`, {
            method: 'DELETE',
            headers: {
                authorization: `Bearer ${localStorage.getItem('accessToken')}`
            }
